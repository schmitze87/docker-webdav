...
if [ ! -e "$HTTPD_PREFIX/conf/user.passwd" ]; then
    touch "$HTTPD_PREFIX/conf/user.passwd"
    ...
    if [ "x$USERNAME" != "x" ] && [ "x$PASSWORD" != "x" ]; then
        if [ "$AUTH_TYPE" = "Digest" ]; then
            ...
            printf '%s\n' "$USERNAME:$REALM:$HASH" > $HTTPD_PREFIX/conf/user.passwd
        else
            htpasswd -B -b -c "$HTTPD_PREFIX/conf/user.passwd" $USERNAME $PASSWORD
        fi
    fi
fi
...
if [ "${SSL_CERT:-none}" = "selfsigned" ]; then
    ...
    if [ ! -e $HTTPD_PREFIX/conf/privkey.pem ] || [ ! -e $HTTPD_PREFIX/conf/cert.pem ]; then
        openssl req -x509 -newkey rsa:2048 -days 1000 -nodes \
            -keyout $HTTPD_PREFIX/conf/privkey.pem -out $HTTPD_PREFIX/conf/cert.pem -subj "/CN=${SERVER_NAME:-selfsigned}"
    fi
fi
